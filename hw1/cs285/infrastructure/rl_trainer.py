...
MAX_NVIDEO = 2 # 2
MAX_VIDEO_LEN = 1000 # 40  # we overwrite this in the code below
...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if(itr == 0):
            ...
            # loaded_paths = pickle.load(str(load_initial_expertdata))
            print(len(loaded_paths[0]))
            print()
            return loaded_paths, 0, None
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            #     # print(train_step)
            #     # frames.append(self._label_with_episode_number(frame, episode_num=train_step))
            #     frame = self.env.render('human')
        ...
        # imageio.mimwrite(os.path.join('./cs285/videos/', 'ant-v2-learner.gif'), frames, fps=60)
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            labels = expert_policy.get_action(paths[i]["observation"])
            paths[i]["action"] = labels
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, training_logs):
        ...
        if self.log_video and train_video_paths != None:
            ...
            eval_video_paths = utils.sample_n_trajectories(self.env, eval_policy, MAX_NVIDEO, MAX_VIDEO_LEN, True, render_mode=('rgb_array'))
