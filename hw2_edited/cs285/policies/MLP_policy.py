...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs):
        with torch.no_grad():
            ...
            action, prob = self.sample_action(observation)

            # TODO return the action that the policy prescribes
        return ptu.to_numpy(action), ptu.to_numpy(prob)  #action
    ...
    # def predict_probs(self, observation, ac):
    #     if self.discrete:
    #         logits = self.logits_na(observation)
    #         probs = F.softmax(logits, dim = -1)
    #         prob = ptu.to_numpy(probs[ac])
    #         return prob
    #     else:
    ...
    #         prob = ptu.to_numpy(torch.exp(log_prob).unsqueeze(0))
    #         return prob
    #
